...
def compute_baseline_score(features: FeatureVector, 
    ...
    # Note: We separate file_hist weight in V2 baseline if desired, 
    # or combine them. User prompt suggested separating file_historical_risk_score.
    
    dep_score = features.get("dependency_risk_score", 0.0)
    hist_score = features.get("historical_risk_score", 0.0) # Global/Bucket
    
    # 2. Weighted Sum
    # raw = w_churn * churn + w_crit * crit + ...
