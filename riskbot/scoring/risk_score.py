...
from riskbot.config import RISK_THRESHOLD_HIGH
from riskbot.features.types import FeatureVector
from riskbot.scoring.types import RiskResult
from riskbot.scoring import baseline, thresholds, calibration, model
...
class RiskScorer:
    ...
    def calculate_score(self, features: FeatureVector, evidence: List[str] = None) -> RiskResult:
        ...
        # Specificity via Explanation Layer:
        # We rely on the explanation summary to list exact matched patterns (e.g. "Touched critical paths: api/")
        # So we keep scoring reasons high-level.
        ...
        # Use raw blast radius if available? 
        # For now, we rely on dependency score but GUARD against 0 impact message
        # Ideally we pipe 'downstream_count' via features if possible?
        # But 'dependency_risk_score' implies radius > 0 if score > 0
        if components["dependency"] >= 0.5:
            ...
            pass # Explicitly do nothing for 0 services
        ...
        # FULL = real GitHub data, FALLBACK = minimal/mock data
        # PARTIAL = real data but missing critical signals (like files)
        data_quality = "FULL"
        ...
        if raw_score <= 1 and len(reasons) == 0:
            ...
            evidence_items.append("⚠️ No files fetched (churn=0.0)")
        ...
        result: RiskResult = {
            ...
            "decision": decision,
            "reasons": reasons[:3],
            "evidence": all_evidence[:5],  # Top 5 evidence items
            "model_version": model_version,
            "feature_version": features.get("feature_version", "unknown"),
            "components": components,
            "data_quality": data_quality  # NEW: indicates real vs fallback data
        }
    ...
    def calculate_score_with_explanation(self, features: FeatureVector, raw: Any = None, evidence: List[str] = None):
        ...
        Args:
            ...
            raw: RawSignals (optional, for evidence)
            evidence: Additional evidence (optional)
            
        Returns:
            Tuple of (RiskResult, ExplanationReport)
        """
        from riskbot.explain import explainer as exp
        
        # Calculate risk
        result = self.calculate_score(features, evidence)
        
        # Generate explanation
        raw_signals = raw or {}  # Fallback to empty if not provided
