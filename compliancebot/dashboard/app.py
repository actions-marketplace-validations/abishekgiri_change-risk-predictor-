...
kpi2.metric("High Severity", high_risk_prs, delta_color="inverse")
kpi3.metric("Avg Severity Score", f"{avg_score:.1f}")
...
with col_charts2:
    st.subheader("Severity Score Trend")
    if not df.empty:
...
if unlabeled_df.empty:
    ...
    unlabeled_df['display'] = unlabeled_df.apply(
        lambda x: f"{x['repo']} #{x['pr_number']} (Severity: {x['risk_score']})", axis=1
    )
...
# Note: Column names kept as 'risk_*' for DB compatibility, but displayed as 'Severity'
...
if not df.empty:
    st.dataframe(
        ...
        column_config={
            ...
            "repository": "Repo",
            "risk_score": st.column_config.ProgressColumn("Severity Score", min_value=0, max_value=100, format="%d"),
