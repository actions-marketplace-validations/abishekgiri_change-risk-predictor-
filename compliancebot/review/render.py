...
def to_markdown(result: ReviewPriorityResult) -> str:
    ...
    # Priority Indicator
    priority_map = {"P0": "[CRITICAL]", "P1": "[HIGH]", "P2": "[NORMAL]"}
    indicator = priority_map.get(result.priority, "[INFO]")
    
    lines.append(f"{indicator} **Review Priority: {result.priority} — {result.label}**")
    lines.append("")
    lines.append("**Reason:**")
